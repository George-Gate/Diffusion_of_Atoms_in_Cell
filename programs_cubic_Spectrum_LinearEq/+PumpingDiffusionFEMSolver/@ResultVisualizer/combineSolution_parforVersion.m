...
% lineID should be an integer
    ...
    switch sampleDim
        case '1D'
            ...
            % get u, each row of u stores the solution of one time point.
            u=obj.sol_u;
            % get coordinate list
       ...
       % ------------------------ combine solution --------------------------
            lenT=size(u,1);
            rho=zeros(length(xList),lenT,dimRho);
            u_dim=permute(reshape(u,lenT,Nbasis,dimRho),[1 3 2]);   % u_dim(time,iDim,iNo)
