...
problem.rho_b_ph=ones(8,1)/8/L^3;
% set boundary for each plane (x=-1, x=+1, y=-1, y=+1, z=-1, z=+1)
problem.drho_b_ph={zeros(8,1);   % constant
...
problem.robinA_ph=ones(8);
% constant number for all boundary
problem.robinA_ph=1;
% set boundary for each plane (x=-1, x=+1, y=-1, y=+1, z=-1, z=+1)
problem.robinA_ph={{@(y,z)y+z};      % scalar 2D function
...
problem.robinA_ph={ones(8);          % constant matrix
